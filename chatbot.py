...
with st.sidebar:
    server_url = st.text_input("服务 graphql-server/go-server 请求地址, 默认为 http://arcadia-portal-server.arcadia.svc:8081/chat", key="url")
...
if not server_url:
    server_url = "http://arcadia-portal-server.arcadia.svc:8081/chat"

for msg in st.session_state.messages:
    st.chat_message(msg["role"]).write(msg["content"])

if prompt := st.chat_input():
    response = requests.post(server_url, json={"query":prompt,"response_mode":"blocking","conversion_id":conversion_id,"app_name":"base-chat-english-teacher", "app_namespace":"arcadia"})
